...
MemoryManager::MemoryManager() : freep(0), pool_free_pos(0) {
    init();
}
...
MemoryManager::mem_header_t* MemoryManager::get_mem_from_pool(unsigned int nquantas) {
...
void* MemoryManager::alloc(unsigned int nbytes) {
    mem_header_t* p;
    mem_header_t* prevp;
    ...
    unsigned int nquantas = (nbytes + sizeof(mem_header_t) - 1) / sizeof(mem_header_t) + 1;
    ...
    for (p = prevp->s.next; ; prevp = p, p = p->s.next) {
        ...
        // memory
            ...
            if ((p = get_mem_from_pool(nquantas)) == 0) {
                /* Allocation fails, TODO: call terminate */
            }
        }
    }
}
...
void MemoryManager::free(void* ap) {
    mem_header_t* block;
    mem_header_t* p;
